...
export function ScrollDownToast(prop: { show: boolean; onclick?: () => void; scrollToLastAIMessage?: () => void }) {
  const [isAtLastMessage, setIsAtLastMessage] = useState(false);

  useEffect(() => {
    const checkIfAtLastMessage = () => {
      const chatBody = document.querySelector(`.${styles["chat-body"]}`);
      if (!chatBody) return;

      const lastAIMessage = chatBody.querySelector(`.${styles["chat-message"]}:last-child`);
      if (!lastAIMessage) return;

      const rect = lastAIMessage.getBoundingClientRect();
      const chatBodyRect = chatBody.getBoundingClientRect();
      
      // Check if the last message is fully visible
      setIsAtLastMessage(
        rect.bottom <= chatBodyRect.bottom && 
        rect.top >= chatBodyRect.top
      );
    };

    const chatBody = document.querySelector(`.${styles["chat-body"]}`);
    if (chatBody) {
      chatBody.addEventListener('scroll', checkIfAtLastMessage);
      // Initial check
      checkIfAtLastMessage();
    }

    return () => {
      if (chatBody) {
        chatBody.removeEventListener('scroll', checkIfAtLastMessage);
      }
    };
  }, []);
  ...
  return (
    <div
      className={
        ...
        (prop.show ? ` ${styles["show"]}` : "") +
        (isAtLastMessage ? ` ${styles["hide"]}` : "")
      }
    >
      <div
        className={styles["toast-content"]}
        onClick={() => {
          if (prop.scrollToLastAIMessage) {
            prop.scrollToLastAIMessage();
...
function useScrollToBottom(
  ...
  // Функция для умной прокрутки к последнему сообщению AI
  function scrollToLastAIMessage() {
    const dom = scrollRef.current;
    if (dom) {
      requestAnimationFrame(() => {
        // Находим все сообщения от AI
        const aiMessages = dom.querySelectorAll(`.${styles["chat-message"]}`);
        if (aiMessages.length > 0) {
          // Берем последнее сообщение от AI
  ...
  // Автоматическая прокрутка только для новых сообщений от ассистента
  useEffect(() => {
    if (shouldScroll && !detach) {
      scrollDomToBottom();
      setShouldScroll(false);
    }
  }, [shouldScroll, detach]);

  // Сброс позиции скролла при открытии нового чата
  useEffect(() => {
    const dom = scrollRef.current;
    if (dom) {
      dom.scrollTo(0, 0);
...
function _Chat() {
  ...
  const isScrolledToBottom = scrollRef?.current
    ...
    : false;
  const { setAutoScroll, scrollDomToBottom, setShouldScroll, scrollToLastAIMessage } = useScrollToBottom(
    scrollRef,
    isScrolledToBottom,
  );
  ...
  const onChatBodyScroll = (e: HTMLElement) => {
    ...
    // Находим последнее сообщение AI
    const aiMessages = e.querySelectorAll(`.${styles["chat-message"]}`);
    if (aiMessages.length > 0) {
      const lastAIMessage = aiMessages[aiMessages.length - 1] as HTMLElement;
      const lastAIMessageBottom = lastAIMessage.offsetTop + lastAIMessage.offsetHeight;
      
      // Если прокрутили ниже последнего сообщения AI, возвращаем к нему
      if (e.scrollTop > lastAIMessageBottom - e.clientHeight) {
        e.scrollTop = lastAIMessageBottom - e.clientHeight;
      }
    }
  };

  // Добавляем обработчик колесика мыши для дополнительного контроля
  const onWheel = (e: WheelEvent) => {
    const chatBody = scrollRef.current;
    if (!chatBody) return;

    const aiMessages = chatBody.querySelectorAll(`.${styles["chat-message"]}`);
    if (aiMessages.length > 0) {
      const lastAIMessage = aiMessages[aiMessages.length - 1] as HTMLElement;
      const lastAIMessageBottom = lastAIMessage.offsetTop + lastAIMessage.offsetHeight;
      
      // Если пытаемся прокрутить ниже последнего сообщения AI
      if (chatBody.scrollTop + chatBody.clientHeight >= lastAIMessageBottom && e.deltaY > 0) {
        e.preventDefault();
      }
    }
  };

  // Добавляем эффект для установки обработчика колесика мыши
  useEffect(() => {
    const chatBody = scrollRef.current;
    if (chatBody) {
      chatBody.addEventListener('wheel', onWheel, { passive: false });
      return () => {
        chatBody.removeEventListener('wheel', onWheel);
      };
    }
  }, []);
  ...
  useEffect(() => {
    // Если последнее сообщение от AI и оно только что закончило стримиться
    ...
    <div className={styles.chat} key={session.id}>
                ...
                <div className={styles["chat-message-container"]}>
                  ...
                  <div className={styles["chat-message-item"]}>
                    <Markdown
                      ...
                      loading={showTyping && message.content.length === 0 && !isUser}
      ...
      <div className={styles["chat-input-panel"]}>
        <ScrollDownToast
          ...
          show={!hitBottom}
          scrollToLastAIMessage={scrollToLastAIMessage}
        />
